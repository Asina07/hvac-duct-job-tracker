...
const UpdateJobPage = () => {
  ...
  // When job loads, fill the form
  useEffect(() => {
    if (job) {
      setFormData({
        date_received: job.date_received?.split("T")[0] || "",
        job_number: job.job_number || "",
        item: job.item || "",
        material_id: job.material_id || "",
        project_id: job.project_id || "",
        level: job.level || "",
        total_sqm: job.total_sqm || "",
        unit: job.unit || "SQM",
        status_id: job.status_id || "",
        date_to_production: job.date_to_production?.split("T")[0] || "",
        notes: job.notes || "",
      });
    }
  }, [job]);
  ...
  return (
    <LayoutComp mainHeader={"Update Jobs"}>
      <section className="bg-white dark:bg-gray-900">
        <div className="flex justify-between items-center mb-6">
          <h1 className="text-2xl font-bold text-gray-800">Edit Jobs</h1>
